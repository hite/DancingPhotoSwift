//
//  DPLayoutSnapKitController.swift
//  DancingPhotoSwift
//
...
class DPLayoutSnapKitController: DPLayoutPageController {
        ...
        for (yIdx, xColors) in colorMatrix.enumerated() {
            ...
            for (i, color) in xColors.enumerated() {
                ...
                dot.snp.makeConstraints { (make) in
                    make.left.equalTo(xStartOffset + (xGap + xStep) * CGFloat(i))
                    make.top.equalTo(yStartOffset + CGFloat(yIdx) * (yStep + yGap))
                    make.width.equalTo(xStep)
                    make.height.equalTo(yStep)
                }
            }
        }
